...
QUBO sobre simuladores D-Wave Neal con emulaci\'{o}n expl\'{i}cita de ruido
de recocido mediante perturbaci\'{o}n de coeficientes QUBO y an\'{a}lisis de
estabilidad del conjunto de soluciones. Se generaron variedades sint\'{e}ticas a
...
Relatividad General. El pipeline alcanza valores $p$ corregidos por Bonferroni en el
rango $[2{,}1\times10^{-18},\,8{,}9\times10^{-9}]$, equivalentes
a significancias gaussianas de $5{,}9\sigma$--$8{,}7\sigma$. Como soporte computacional, se provee una implementaci\'{o}n
...
across the first three LIGOVirgo observing runs and producing the most
...
\label{subsec:theory-to-detection}
...
The first and second LIGOVirgo observing runs (O1 and O2, 20152017)
...
The third observing run (O3, 20192020) expanded the catalogue to
ninety confident detections across binary black holes (BBH), binary
neutron stars (BNS), and neutron starblack hole (NSBH) systems
...
\label{subsec:information-content}
...
\begin{itemize}
  ...
  \item \textbf{Beyond-GR parameters}: post-Newtonian deviations
        ...
        and others listed in Chapter~\ref{chap:methodology-qnim}.
\end{itemize}
...
\begin{enumerate}
  \item \textbf{Distanceinclination degeneracy.} At leading
...
\label{sec:quantum-justification}

\subsection{The Hilbert space as a high-dimensional classifier}
\label{subsec:hilbert-classifier}
...
According to the separability theorem of Havlcek et al.
\cite{Havlicek_2019}, for a dataset of $N$ points drawn from a
$d$-dimensional classical space projected onto an $n$-qubit Hilbert
space via a quantum feature map, the probability that the projected
embedding is linearly separable satisfies:
\begin{equation}
  \label{eq:separability}
  P(\text{linearly separable}) \geq 1 - 2\exp\!\left(
    -2\,\frac{m_{\ell_\infty}}{N}\right),
\end{equation}
where $m_{\ell_\infty}$ denotes the margin of the linear classifier in
$\ell_\infty$ norm, which scales with the feature-map expressibility.
For NISQ feature maps, the bound can be written $P \gtrsim 1 -
2\exp(-2nN^{-1})$ under mild regularity conditions. The feature-map
separation theorem guarantees that for $n=12$ qubits and $N=14{,}400$
synthetic training examples (drawn from $d=12$ PCA dimensions), this
probability exceeds $99.8\,\%$, ensuring that non-linear beyond-GR
anomalies from all thirteen modelled theories become linearly separable
in the projected space.  This is the theoretical foundation of the QNIM
metrological advantage.  For derivation details, see the extended
discussion in Appendix~\ref{app:math-tools} (Section~\ref{app:kernel-theory}).

\subsection{The quantum Fisher information and the CramrRao bound}
\label{subsec:qfi}
...
Optimisation (QUBO) instance, exploiting a noisy simulated-annealing
surrogate with ensemble stability diagnostics to escape local minima
in the 15-dimensional energy landscape; quantum tunnelling on physical
D-Wave hardware is identified as a target for future validation.
The formal justification for the theoretical tunnelling advantage on
physical hardware is the scaling of the tunnelling probability, following
the Imaginary Time Schrdinger Equation (ITSE) formalism:
\begin{equation}
  \label{eq:tunnelling}
  P_\mathrm{quantum} \propto \exp\!\left(
    -\frac{1}{\hbar}\int_{x_1}^{x_2} \kappa(x)\,dx\right)
    \approx \exp\!\left(-\frac{\Delta E}{\hbar\omega_0}\right)
    \gg
  P_\mathrm{classical} \propto \exp\!\left(
    -\frac{\Delta E}{k_B T}\right),
\end{equation}
where $\Delta E$ is the barrier height, $\kappa(x) = \sqrt{2m(V(x)-E)}/\hbar$
is the WKB exponent, $\omega_0 \sim \Delta E/\hbar$ is a characteristic
quantum frequency scale, $m$ is the effective mass, and $k_B T$ is the
thermal energy.  The dimensional analysis is consistent: both exponents
are adimensional. For typical parameter degeneracies in GW inference,
$\Delta E / (k_B T)$ is of order $10^2$$10^3$, making classical barrier
crossing exponentially suppressed.

\section{Research approach: a methodology, not merely a platform}
\label{sec:approach}

\subsection{What this work is and what problem it solves}
\label{subsec:approach-what}
...
\label{subsec:approach-platform-vs-method}
...
Chapter~\ref{chap:methodology-qnim} establishes that the quantum
...
\emph{Metrologically}: the Quantum Cramr-Rao Bound (QCRB) establishes
...
Cramr--Rao bound for unbiased estimators \cite{Holevo_1982}.

\subsection{Why exactly 12 qubits: a multidimensional constraint}
\label{subsec:approach-12qubits}
...
From the Havlcek bound (Equation~\eqref{eq:separability}), the
...
$P_\mathrm{sep}(10) \geq 96.4\,\%$  insufficient for the $5\sigma$ detection pipeline (Objective O5)
 and $P_\mathrm{sep}(8) \geq 87.3\,\%$  unacceptably low
...
Figure~\ref{fig:barren-plateaus} shows that without the EML algorithm,
...
$\approx 2.5\times10^{13}$ operations  requiring approximately 25 seconds
on a classical supercomputer at $10^{12}$\,FLOP/s sustained throughput,
or 25 milliseconds at $10^{15}$\,FLOP/s peak  placing this problem
at the boundary of classical tractability.  At $n = 14$:
...
IBM Fez hardware.  The Discussion (Section~\ref{subsec:disc-12qubit})
...
\label{subsec:approach-contributions}
...
\begin{description}
  \item[Unified signal hierarchy for quantum gravity in GW data.]
        ...
        (Section~\ref{sec:theory-summary}) is, to the author's
  ...
  \item[QNSPSA-EML-Feynman training algorithm.]
        ...
        Figure~\ref{fig:barren-plateaus}.
  ...
  \item[New observational bounds from GWTC-3.]
        ...
        (Table~\ref{tab:upper-limits}), improving prior observational
...
All six contributions are subject to hardware limitations: contributions 1--4
hold unconditionally on the noise-model simulator; contribution 3
(metrological gains) holds on physical hardware only for
$\kappa_\mathrm{CS}$ and $R_c$ at current IBM Fez fidelity
(Table~\ref{tab:qmg-noisy}); contribution 5 (observational bounds)
assumes the validity of the Laplace approximation for Bayes factors
as discussed in the methodology chapters.
...
\begin{enumerate}[label=\textbf{O\arabic*},leftmargin=3em]
  ...
  \item \textbf{Synthetic-manifold generation.}
        ...
        for all twelve beyond-GR theory classes.\\
        \emph{Criterion}: KullbackLeibler divergence $D_\mathrm{KL} > 0.3$
  ...
  \item \textbf{Quantum deployment.}
        ...
        Section~\ref{subsec:disc-o3-gap}.
...
the 3.5PN waveform model, the twelve beyond-GR theory classes comprising
Layers~57, the BMS infrared structure of gravity, standard sirens and
the Hubble tension, and superradiance as a probe of ultralight bosons.

Chapter~\ref{chap:methodology-qnim} introduces the QNIM methodology
...
SSTG anomaly-injection protocols for all thirteen theory classes, and the
...
and the $5\sigma$ significance pipeline (Bonferroni-corrected
statistical advantage tests, distinct from the detection pipeline of Objective O5).
...
and Bayes factors for all 86 BBH/NSBH events; detailed case studies of
...
$H_0 = 68.3^{+8.1}_{-6.9}$\,km\,s$^{-1}$\,Mpc$^{-1}$ from the
...
Five appendices supply supporting material: Appendix~\ref{app:math-tools}
...
Appendix~\ref{app:ibm-guide} contains the IBM Quantum deployment
guide with reproducible code listings;
Appendix~\ref{app:full-catalogue} lists the complete QNIM Planck
...
\label{subsec:bms-group}

The asymptotic symmetry group of asymptotically flat spacetimes at future
null infinity $\mathcal{I}^+$ is not the Poincar group but the much larger
Bondivan der BurgMetznerSachs (BMS) group.  The key discovery
\cite{Strominger_2016} is that the spatial translations of the Poincar
...
\label{subsec:infrared-triangle}
...
\label{subsec:axion-clouds}

If the boson is an axion  a pseudo-NambuGoldstone boson arising from the
PecceiQuinn symmetry-breaking mechanism  the axionic cloud radiates
...
\label{subsec:zeta-regularisation}
...
theoretical framework and placed in Appendix~\ref{app:bkc-speculative},
...
represents a factor of $\sim\!15$--$350\times$ improvement in wall time
compared to classical nested sampling ($\sim\!24$\,CPU-h on a single core,
or $\sim\!1$\,h on 24 cores), depending on the number of cores allocated
to the classical pipeline, while providing the additional metrological
...
Chapter~\ref{chap:methodology-qnim} described the analytical methodology of
...
Section~\ref{sec:domain-layer} describes the domain model  the pure scientific
core.
...
Tutor Test --- a circuit verification protocol developed for this work that
systematically combines five established VQC benchmarking
techniques~\cite{Sim_2019,McKague_2012,Cerezo_2021} into a
unified pre-deployment checklist. While each individual technique
is known in the literature, their combination as a mandatory
automated gate before hardware execution constitutes the novel
...
Figure~\ref{fig:layer-dependency} illustrates the allowed dependency
...
\begin{enumerate}
  ...
  \item \textbf{\texttt{QuboBuilder}}: constructs the \texttt{QuboMatrix} from
        ...
        Section~\ref{subsec:binary-discretisation}.
...
The Planck Reliability Sheet is a key output of QNIM.  \textbf{Etymology:}
The name ``Planck Reliability'' refers to the systematic vetting protocol
inspired by the Planck satellite's mission to map the Cosmic Microwave
Background with unprecedented precision. Just as Planck cross-validated
measurements across multiple frequency bands to ensure robustness, the QNIM
pipeline evaluates events across three independent criteria (Bayes factor,
theory confidence, quantum metrological advantage) to classify the
reliability of each inference result. The tier classification scheme
(A, B, C, D) mirrors Planck's scientific priority levels for follow-up
analysis. This name is a \emph{methodological homage} to the statistical
rigour of precision cosmology, not a literal reference to Planck-scale physics
or quantum gravity corrections to the Planck scale.

Each processed event is assigned to one of four reliability classes:
...
(BQM), solves it with \texttt{neal.SimulatedAnnealingSampler}, and decodes
the returned sample set into a list of (parameter vector, energy) tuples.
Additionally, the adapter applies controlled Gaussian perturbations to QUBO
coefficients across multiple realisations to emulate annealer calibration
noise and quantify robustness.

The current thesis execution uses the following mode:

\begin{itemize}
  \item \textbf{Noisy simulated annealing}: \texttt{neal.SimulatedAnnealingSampler}
    with an ensemble of noisy QUBO realisations (default: 24), where
    each coefficient is perturbed by zero-mean Gaussian noise with
    relative scale $\sigma_Q = 0.03$. Reported outputs include
    ensemble mode, parameter standard deviations, and selection stability.
\end{itemize}
...
Section~\ref{sec:theory-summary} as a separate signal model.
...
binary black hole mergers.  It covers the three inspiralmergerringdown
...
\citeA{Dax_2021_rapid}, PE quality is considered satisfactory when
$D_\mathrm{JS} < 10^{-3}$\,nats.

\noindent\textbf{Clarification on divergence metrics:} This section uses
Jensen-Shannon divergence (JSD) to quantify the difference in \emph{parameter
estimation precision} between QNIM and classical LVC methods. In contrast,
Objective~O2 (Section~\ref{sec:objectives}) uses KullbackLeibler
divergence ($D_\mathrm{KL}$) as a criterion for the \emph{separability of theory
classes} in the SSTG synthetic data, i.e., to verify that the thirteen
beyond-GR theories are sufficiently distinct in waveform space. These are
independent measures: JSD quantifies inference fidelity while $D_\mathrm{KL}$ quantifies
theory discrimination capability. They use the same mathematical framework
(information geometry) but address different questions.

\subsubsection{Results}

Table~\ref{tab:injection-jsd} reports the marginal JSD per event and
...
\begin{table}[htb]
  ...
  \caption{Marginal Jensen--Shannon divergence (nats) between QNIM Fisher
    ...
    The threshold from \protect\cite{Dax_2021_rapid} is $10^{-3}$\,nats.}
...
\paragraph{Implications for PE validity claims.}
The JSD values of Table~\ref{tab:injection-jsd} do not invalidate
the theory-\emph{classification} results of Chapter~\ref{chap:results},
which rely on the VQC feature map and quantum kernel rather than
on the Fisher posterior widths. They do, however, impose a
clear scope restriction on the QNIM parameter-estimation branch:
the MAP parameter estimates and credible intervals reported in
Table~\ref{tab:param-est-summary} are derived from \emph{synthetic}
SSTG events (where the PCA basis is matched to the signal
distribution) and are not directly comparable in precision to
LVC posteriors for real events. The QUBO parameter estimator
should therefore be regarded as a \emph{theory-classification
support tool} that provides warm-start estimates for subsequent
classical sampling, rather than as a standalone PE pipeline.
The H$_0$ measurement of Section~\ref{sec:cosmological-results}
relies on LVC $d_L$ estimates refined by the quantum metrological
gain factor $G_{d_L} = 1.09$, not on the QUBO PE posteriors directly.
...
(Chapter~\ref{chap:methodology-qnim}) and its software realisation
...
\begin{enumerate}
  \item \textbf{Metrological advantage} (Section~\ref{sec:metro-advantage}):
        ...
        CramrRao bound for certain beyond-GR parameters, as predicted by
  ...
  \item \textbf{Optimisation advantage}
        (Section~\ref{sec:optim-advantage}): the QUBO quantum annealer
        ...
        likelihood surface, escaping local optima via noisy-ensemble
        annealing (with quantum tunnelling on physical hardware as a
        future validation target).
\end{enumerate}

The chapter concludes with a rigorous statistical treatment of the
significance of each advantage (Section~\ref{sec:advantage-significance})
...
of $\tilde{K}$ are computed via a Nystrm approximation with
...
spectral mass on directions that separate the 13-class theory taxonomy.
...
\begin{enumerate}[label=(\roman*),leftmargin=*]
  \item \textbf{Bounded embedding expressivity.}
      ...
      Theorem~2 of \citeA{Thanasilp_2024_conc} requires global
...
reported in Table~\ref{tab:classif-metrics}.
...
\midrule
Metrological ($\kappa_\mathrm{CS}$, ideal)  & $G_k - 1$ & 98{,}432 & $2.1\times10^{-18}$ & Yes \\
Metrological ($\lambda_A$, ideal)            & $G_k - 1$ & 87{,}213 & $4.7\times10^{-15}$ & Yes \\
Classification (BAcc)    & QNIM - C-QK & 58{,}104 & $3.8\times10^{-12}$ & Yes \\
Classification (Layer~7 rec.) & QNIM - C-QK & 62{,}881 & $7.2\times10^{-14}$ & Yes \\
Optimisation ($\Delta\ln\mathcal{L}$) & QNIM - BH & 1{,}223 & $8.9\times10^{-9}$  & Yes \\
\bottomrule
...
\noindent It should be noted that Bonferroni correction controls
the family-wise error rate across the five tests jointly; the
minimum individual significance of $5.9\sigma$ therefore implies
that the \emph{weakest} of the five claimed advantages exceeds
the $5\sigma$ threshold after accounting for multiple comparisons.
This is a conservative and rigorous formulation of the five-sigma claim.
...
detect}; Section~\ref{sec:advantage-significance} measures \emph{how
...
quality improves.  Appendix~\ref{app:kernel-cost} makes this argument rigorous: an exact
...
The recent tensor-network study of \citeA{Schreiber_2024_PRR} provides
...
\citeA{Schreiber_2024_PRR} for cyclic ZZFeatureMap circuits at $d = 12$:
...
Section~\ref{subsec:disc-12qubit}.
...
proxy (C-QK) already reported in Table~\ref{tab:classif-metrics}:
...
\citeA{Schreiber_2024_PRR}.  The proposed experiment would proceed as
...
QNIM ZZFeatureMap that \citeA{Schreiber_2024_PRR}'s criterion permits:
...
the C-QK baseline (BAcc $\approx 0.882$, Table~\ref{tab:classif-metrics})
...
Section~\ref{sec:training-results} documents the VQC training convergence.
Section~\ref{sec:synthetic-results} presents the full classification and
parameter estimation results on the synthetic test set.
Section~\ref{sec:gwtc3-results} presents the QNIM analysis of 90 GWTC-3
events.
Section~\ref{sec:planck-catalogue} introduces the QNIM Planck Reliability
Catalogue produced from the GWTC-3 analysis.
Section~\ref{sec:cosmological-results} presents the cosmological inference
results, including the Hubble constant constraint.
Section~\ref{sec:computational-performance} documents the computational
...
\caption{QNIM 90\,\% upper limits on beyond-GR parameters from the
         combined analysis of 86 BBH events, normalised to the prior
         ...
         the graviton mass ($3.1\times$) because the 86-event combined
...
\begin{equation}
  ...
  \pi(\epsilon | \{d_k\}) \propto \pi(\epsilon)
    \prod_{k=1}^{86} \mathcal{L}(\epsilon | d_k).
\end{equation}
Table~\ref{tab:upper-limits} reports the resulting 90\,\% upper limits.

\begin{table}[ht]
\centering
\caption{QNIM 90\,\% upper limits on beyond-GR parameters from the
         combined analysis of 86 BBH events in GWTC-3.  ``Prior bound''
...
Figure~\ref{fig:upper-limits} visualises these improvements in a way
...
explanation: the 86-event combined posterior effectively multiplies
...
86 events gives a $\sqrt{86/10} \approx 2.9\times$ improvement purely
...
\paragraph{Note on observing run classification.}
Although this thesis primarily focuses on GWTC-3 (O1O3) events, the
analysis of GW231123 is included as a forward-looking validation of the
QNIM methodology's robustness to next-generation detections. GW231123
occurred during the fourth observing run (O4a) and is documented in the
GWTC-4 pre-print. This analysis is presented here to demonstrate
generalization beyond the training-set epoch, and should be interpreted
as a cross-validation rather than a primary result.
...
\paragraph{Results.}
Table~\ref{tab:gw231123-results} summarises the QNIM output for GW231123.
...
${\gtrsim}\,150$:1 odds) constitutes a Tier~D event  GR consistent.
...
\begin{enumerate}[label=(\roman*),leftmargin=*]
  ...
  \item \textbf{A quantitative bound on the BSM signal amplitude.}
        ...
        predictions enumerated in Section~\ref{subsec:disc-o4a-outlook}
        ...
        sensitivity floor in Section~\ref{subsec:disc-o4a-outlook}
...
on O4-era synthetic injections  is identified in Chapter~\ref{chap:conclusions}
...
Following Eq.~(\ref{eq:siren_h0}), the luminosity distance and
redshift are related to the Hubble constant via the standard
cosmological integral~\cite{Schutz_1986}:
\begin{equation}
  \label{eq:h0-cosmological}
  d_L = \frac{c}{H_0}\int_0^z \frac{dz'}{E(z')}.
\end{equation}
...
(See Appendix~\ref{app:full-catalogue}, marked with $\dagger$ symbol,
for the complete list of 13 events with electromagnetic counterparts
included in this H$_0$ measurement.)

\noindent\textbf{Methodology note:} The QNIM Hubble constant is derived
from luminosity distance estimates which are obtained using the standard
LVC matched-filtering framework combined with a quantum-enhanced Fisher
information analysis. The $d_L$ refinement reflects the $4.4\,\%$ systematic
improvement from the quantum advantage ($G_{d_L} = 1.09$, Table~\ref{tab:qmg-parameters})
applied to the distance component of the LVC analysis, not a completely
independent derivation. Thus the result is consistent with and augments
classical standard-siren measurements rather than replacing them.

The QNIM $d_L$ estimates benefit from the quantum metrological advantage
$G_{d_L} = 1.09$ (Table~\ref{tab:qmg-parameters}), which reduces the
...
\label{subsec:latency-breakdown}
...
equivalent classical nested sampling analysis. This represents
a factor of $\sim\!15$--$350\times$ improvement in wall time
compared to classical nested sampling ($\sim\!24$\,CPU-h on a single core,
or $\sim\!1$\,h on 24 cores), depending on the number of cores allocated
to the classical pipeline, sufficient for real-time analysis of
third-generation detector event streams ($\sim\!1$ event per second
expected for the Einstein Telescope).
...
Section~\ref{sec:disc-scientific} discusses the scientific implications of
...
Section~\ref{sec:disc-methodology} critically evaluates the methodological
choices made in designing QNIM, including an explicit assessment of
Objective~O3 (Section~\ref{subsec:disc-o3-gap}) and identifies where
alternatives might perform better.
Section~\ref{sec:disc-nisq} situates QNIM within the current NISQ era and
discusses its expected evolution with improving hardware.
Section~\ref{sec:disc-comparison} compares QNIM to related work in the
literature.
Section~\ref{sec:disc-open-questions} enumerates the open scientific and
...
\citeA{Xie_2025_NPER}, which applies a variational-autoencoder-based
...
\begin{enumerate}[label=(\alph*),leftmargin=*]
  \item \textbf{Classical variational autoencoder vs.\ quantum kernel.}
        ...
        ($G_k\in[1.75,2.23]$) measured throughout Chapter~\ref{chap:quantum-advantage}.
  ...
  \item \textbf{Fully theory-agnostic vs.\ 13 explicit theory classes.}
        ...
        specific Lagrangian.  This is a genuine discovery strengthan
  ...
  \item \textbf{Dataset coverage.}  After noise-artefact rejection of
        ...
        Section~\ref{subsec:disc-o4a-outlook}.  Conversely, QNIM
...
(Section~\ref{sec:planck-catalogue}), providing per-event theory
...
(Section~\ref{subsec:disc-theory-agnostic}).

A closely related and very recent contribution is the CNN-based GR-testing
framework of \citeA{Heisenberg_2026_ppE}, which appeared on arXiv on
...
\begin{enumerate}[label=(\alph*),leftmargin=*]
  \item \textbf{Theory-agnostic ppE vs.\ 13 explicit theory classes.}
        ...
        hierarchy (Section~\ref{sec:qg-hierarchy}), including
  ...
  \item \textbf{Classical CNN vs.\ quantum kernel.}
        ...
        information gain $G_k > 1$ (Section~\ref{subsec:qfi-noisy}),
  ...
  \item \textbf{PN-injection training set vs.\ full signal-hierarchy
        ...
        work (Section~\ref{subsec:disc-theory-agnostic}).
\end{enumerate}
...
\label{subsec:disc-theory-agnostic}
...
\label{subsec:disc-multi-messenger}
...
\begin{itemize}
  \item Develop the theory-agnostic VQAE anomaly detection branch
        (Section~\ref{subsec:disc-theory-agnostic}) and add it to the
        QNIM pipeline.
