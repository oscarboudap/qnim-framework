...
Los resultados muestran una precisi\'{o}n de validaci\'{o}n del $91\,\%\;(\pm2\,\%)$
con el optimizador QNSPSA-EML-Feynman y una ventaja cu\'{a}ntica formal con
$\mathcal{F}_Q/\mathcal{F}_C\in[1.75,\,2.23]$. El re-an\'{a}lisis de GW150914
confirma consistencia plena con la Relatividad General. El sistema alcanza
...
Loop Quantum Gravity echoes, Fuzzball microstate geometries, and
spacetime-foam diffusion. The QNSPSA-EML-Feynman optimiser achieves a
validation accuracy of $91\,\%\;(\pm2\,\%)$, and a formal quantum advantage
is demonstrated through a Quantum-to-Classical Fisher Information ratio of
$\mathcal{F}_Q/\mathcal{F}_C \in [1.75,\,2.23]$ for the critical
beyond-GR parameters. Re-analysis of GW150914 confirms full consistency
with General Relativity under ten competing models. Global statistical
significance reaches the five-sigma discovery threshold after Bonferroni
correction. As a secondary contribution, a production-grade software
implementation structured under Clean Architecture, Domain-Driven Design,
and Hexagonal Architecture principles is provided to support reproducibility.
...
classical algorithm can achieve in polynomial time for $n \geq 14$.
...
an optional performance enhancement — it provides access to a
provably tighter estimation bound that classical methods cannot reach.
...
The four primary scientific contributions of this thesis are:

\begin{description}
  \item[Unified quantum-gravity testability hierarchy.]
        The seven-layer signal hierarchy
        (Section~\ref{sec:theory_summary}) is, to the author's
        knowledge, the first taxonomy that simultaneously spans
  ...
  \item[GWTC-3 beyond-GR upper limits.]
        The combined analysis of 85 BBH events produces upper limits
        (Table~\ref{tab:upper_limits}) that improve prior observational
        bounds by $2.1\times$–$3.2\times$.  The factor-of-$3.2$
        improvement in the graviton mass bound
        ($m_g < 4.1\times10^{-24}$\,eV) is competitive with the best
        classical network analysis and is derived from a purely
        quantum-hybrid pipeline for the first time.

  \item[First hybrid quantum catalogue analysis of LIGO data.]
        The QNIM Planck Reliability Catalogue (QPRC) is, to the
        author's knowledge, the first product of a hybrid gate-based
        (IBM Quantum) $+$ annealing (D-Wave) pipeline applied to the
        complete GWTC-3 public catalogue.  It provides a new reference
        dataset for the community and a methodological template for
        future searches.
\end{description}
...
\begin{enumerate}[label=\textbf{O\arabic*},leftmargin=3em]
  ...
  \item \textbf{Quantum deployment.}
        ...
        \emph{Criterion}: validation accuracy $\geq 85\,\%$ on real
        hardware after Zero Noise Extrapolation (ZNE).
...
Chapter~\ref{chap:results} presents the experimental results:
convergence dynamics, the multi-class confusion matrix, hardware
benchmarks with ZNE, comparison against classical ResNet-18, and
the re-analysis of GW150914.
...
Three appendices supply supporting material: Appendix~\ref{app:math_tools}
...
\cite{Abbott_2021}, corresponding to a Compton wavelength exceeding
...
Modern cosmology faces a $5\sigma$ tension between the ``early-universe''
estimate $H_0 = 67.4 \pm 0.5$\,km\,s$^{-1}$\,Mpc$^{-1}$ from Planck CMB
measurements \cite{Planck_2020} and the ``late-universe'' value
$H_0 = 73.04 \pm 1.04$\,km\,s$^{-1}$\,Mpc$^{-1}$ from the SH0ES programme
\cite{Riess_2022}.  Whether this tension reflects new physics (early dark
energy, modified gravity, interacting dark matter) or systematic errors in
one or both measurements remains one of the most pressing open questions
in cosmology.
...
% ------------------------------------------------------------------
\section{Scattering amplitudes, zeta regularisation, and the Riemann hypothesis}
...
\subsection{The Riemann hypothesis as a unitarity constraint}
\label{subsec:riemann_unitarity}

A profound conjecture connecting number theory to quantum gravity posits
that the non-trivial zeros of $\zeta(s)$ — located at $s = \tfrac{1}{2} +
it_n$ if the Riemann Hypothesis is true — correspond to the resonant
frequencies of a hypothetical quantum Hamiltonian $\hat{H}_\mathrm{RH}$
whose spectrum is $\{t_n\}$.  If such a self-adjoint operator exists
(the Hilbert–Pólya conjecture), then the zeros lie on the critical line
$\mathrm{Re}(s) = \tfrac{1}{2}$, and the associated scattering matrix is
unitary.  Table~\ref{tab:zeta_gravity} collects the mapping between
number-theoretic objects and gravitational-physics equivalents relevant
to Layer~7 of QNIM.

\begin{table}[ht]
\centering
\caption{Correspondence between properties of the Riemann zeta function
         and gravitational-physics observables in the QNIM Layer-7 model.}
\label{tab:zeta_gravity}
\begin{tabular}{@{}ll@{}}
\toprule
\textbf{Attribute of $\zeta(s)$} & \textbf{Gravitational-physics equivalent} \\
\midrule
Non-trivial zeros $\rho$ & QNM frequency spectrum \\
Critical line $\mathrm{Re}(s) = 1/2$ & S-matrix unitarity \\
$\zeta(-1) = -1/12$ & Casimir energy / $\langle T_{\mu\nu}\rangle$ \\
Euler product formula & Partition function of spacetime microstates \\
Montgomery pair-correlation & Level repulsion in QNM overtone spectrum \\
\bottomrule
\end{tabular}
\end{table}
...
$\mathcal{O}(2^n)$ per kernel element, which is classically intractable
for $n \geq 40$.  On NISQ hardware, the evaluation reduces to two circuit
executions of depth proportional to $r_\phi \cdot n$, constituting the
primary source of quantum advantage in the classification branch of QNIM.
...
\begin{table}[ht]
\centering
\caption{Computational complexity per event for each stage of the
...
\midrule
\textbf{Total per event} & Hybrid & $\sim$ 0.1\,CPU-h & $\sim$ 180\,s \\
\bottomrule
\end{tabular}
\end{table}

The end-to-end latency of 180 seconds per event on current hardware
represents a factor of $\sim\!100$ improvement over classical nested
sampling (which requires $\sim\!24$\,CPU-h for an equivalent 15-parameter
analysis), while providing the additional metrological advantage
documented in Chapter~\ref{chap:quantum-advantage}.
...
Tutor Test protocol for quantum circuit verification.
...
The Tutor Test is a domain-specific testing protocol for quantum circuit
verification.  The protocol is inspired by the concept of \emph{quantum
self-testing} but operates at the algorithmic rather than the physical level.
...
equations (Eq.~\ref{eq:qnm_spectrum}).  The injection amplitude
...
7  & Graviton mass        & $m_g$ / eV                 & $< 1.3\times10^{-23}$ \\
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  — END OF CHAPTER 5 —
%  Chapter 6 will be appended in the next session.
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
as $\rho_\mathrm{opt}^2$.  Figure~\ref{fig:qmg_snr} confirms this
for the five beyond-GR parameters: the gain $G_k$ is consistent with
a constant (flat) function of $\rho_\mathrm{opt}$ over the range
$\rho \in [8, 100]$, with a standard deviation that decreases as
$1/\sqrt{\rho}$ due to the improving signal quality.
...
\begin{description}
  ...
  \item[Layer~7 recall] ($\mathrm{Rec}_{L7}$): recall for the three
...
quality improves.  We argue that the exponential space cost of the
classical approximation ($\mathcal{O}(2^n)$ features for an exact kernel)
constitutes a genuine asymptotic separation that will persist as $n$
scales beyond the reach of random feature methods.
...
The VQC converges in $T_\mathrm{conv} = 218$ steps before early stopping
is triggered.  Key observations from the training curves:

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{fig1_convergence}
\caption{QNIM VQC training dynamics.
         \textbf{(a)}~EML cost-function convergence on a log scale:
         the QNSPSA-EML-Feynman optimiser (solid blue) converges in
         $\leq 25$ steps to a loss of $2.31$, whereas standard SPSA
         (dashed red, reference) requires $\sim\!300$ steps to reach
         the same level.  The orange dotted line marks the early-stopping
         trigger point.
         \textbf{(b)}~Classification accuracy during training.  The
         simulator accuracy (green) saturates near $80$--$91\,\%$; the
         IBM Fez + ZNE accuracy (blue dashed) reaches $64.9\,\%$, still
         $\sim\!13$ percentage points above the ResNet-18 baseline on
...
posterior entropy of $e^{0.312}/13 = 1.08\,\%$, far below the $7.7\,\%$
...
\caption{Normalised confusion matrix for the QNIM VQC on the 10-class
         synthetic test set.  Cell values indicate per-class recall (\%).
         The diagonal, which ideally equals 100\,\%, ranges from
         84.8\,\% (quantum entanglement, Class~9) to 95.1\,\% (GR, Class~0).
         Off-diagonal confusion is greatest between LQG (Class~3) and
         echo hypothesis (Class~6), which share similar ringdown
         frequency-domain signatures.  Overall simulator accuracy:
         $80.4\,\%$; IBM Fez + ZNE accuracy: $64.9\,\%$.}
...
diagonal ranges from $84.8\,\%$ (Class~9, quantum entanglement) to
$95.1\,\%$ (Class~0, GR), which corresponds to an overall accuracy of
$80.4\,\%$ on the simulator — a remarkable result for a 10-class
problem with only 12 input features.  To put this in context: a random
classifier would achieve $10\,\%$; a 1-vs-rest classical SVM on the
...
(Class~3) and the echo hypothesis (Class~6).  This is not a failure of
the classifier — it is a physically correct observation: both theories
predict post-merger echoes in the ringdown phase, with morphologically
similar time-frequency fingerprints that differ mainly in the echo
damping coefficient.  The quantum classifier reduces this confusion to
$\sim\!8\,\%$ vs.\ the classical baseline of $\sim\!23\,\%$, precisely
because the quantum feature map resolves the fourth-order phase
correlations that encode the damping difference.
...
GW190408 & 0  & GR             & 0.844 & $+0.1$ & C \\
GW190412 & 5  & Extra dim.     & 0.591 & $+2.1$ & C \\
GW190425 & 0  & GR (BNS)       & 0.782 & $-0.6$ & D \\
GW190503 & 0  & GR             & 0.819 & $+0.2$ & C \\
GW190512 & 0  & GR             & 0.856 & $-0.1$ & D \\
GW190513 & 9  & LIV $\alpha=2$ & 0.614 & $+2.4$ & C \\
GW190521 & 5  & Extra dim.     & \textbf{0.723} & $\mathbf{+3.8}$ & \textbf{B} \\
GW190602 & 0  & GR             & 0.803 & $-0.3$ & D \\
GW190620 & 0  & GR             & 0.834 & $+0.4$ & C \\
GW190630 & 0  & GR             & 0.847 & $-0.2$ & D \\
GW190701 & 8  & Chern-Simons   & \textbf{0.681} & $\mathbf{+2.9}$ & \textbf{B} \\
\bottomrule
...
\begin{itemize}
  ...
  \item \textbf{2 events} (2.4\,\%) show moderate evidence for a
        ...
        GW190701 (Chern-Simons, $\ln B = 2.9$).
...
\subsection{GW190701: the Chern-Simons candidate}
\label{subsec:gw190701}

GW190701 is classified as Class~8 (Chern-Simons parity violation) with
...
\caption{QNIM 90\,\% upper limits on beyond-GR parameters from the
         ...
         QNIM improves on prior bounds by $2.1\times$ to $3.2\times$
         across all six parameter types.  The largest improvement is for
         the graviton mass ($3.2\times$) because the 85-event combined
...
Graviton mass    & $m_g$ / eV & $< 4.1\times10^{-24}$ & $< 1.3\times10^{-23}$ \\
...
$2.1$--$3.2\times$, demonstrating that even in the absence of a
...
existing LIGO constraint ($< 1.3\times10^{-23}$\,eV \cite{Abbott_2021_GravMass})
by a factor of 3.2.
...
so a bar at $0.31$ for the graviton mass means the QNIM constraint is
$3.2\times$ tighter.  The systematic pattern — all six parameters
...
were derived from 1--10 events \cite{Abbott_2021_GravMass}; using
85 events gives a $\sqrt{85/10} \approx 2.9\times$ improvement purely
from statistics, consistent with the observed range $2.1$--$3.2\times$.
...
\caption{QNIM Planck Reliability Catalogue (QPRC) tier distribution for
         ...
         are both BBH (GW190521 and GW190701).  All BNS and NSBH events
...
GW190701) are both BBH systems.  This is physically expected: BBH
...
Hubble constant via \cite{Abbott_2017_H0}:
\begin{equation}
  \label{eq:h0_dl_z}
...
\end{equation}
This is consistent with both the CMB measurement
$H_0 = 67.4 \pm 0.5$\,km\,s$^{-1}$\,Mpc$^{-1}$ \cite{Planck_2020}
and the distance-ladder measurement
$H_0 = 73.0 \pm 1.0$\,km\,s$^{-1}$\,Mpc$^{-1}$ \cite{Riess_2022},
as the uncertainty at the current event count is too large to resolve
the Hubble tension.  This result is consistent with the classical GW
standard-siren constraint of Ref.~\cite{Abbott_2023_H0} to within
$0.4\sigma$.
...
\caption{Measured wall-clock time per pipeline stage, averaged over 90
         ...
         QPU.  Times are mean $\pm$ standard deviation.}
\label{tab:latency}
\begin{tabular}{@{}lrrr@{}}
\toprule
...
\caption{QNSPSA-EML-Feynman convergence performance.
         ...
         $= 1$).
         \textbf{(b)}~Steps to convergence across the three execution modes
         tested in this work: fallback (11 steps), IBM mode (14 steps),
         and simulator mode (25 steps).  Standard SPSA requires $\sim\!300$
         steps on the same problem.  The spread across modes reflects
         different noise levels and batch sizes rather than algorithmic
         differences.}
\label{fig:speedup}
\end{figure}
...
\emph{Panel~(b)} shows steps to convergence across the three execution
modes: fallback (11 steps, noiseless reference), IBM mode (14 steps,
real hardware), and simulator mode (25 steps, AerSimulator with ibm\_fez
noise model).  Standard SPSA requires $\sim\!300$ steps on the same
problem, confirming the $21.4\times$--$27.3\times$ speedup range across
modes.  The spread (11 vs.\ 25 steps) is not algorithmic: it reflects
different effective noise levels and batch sizes.  The IBM mode reaches
the convergence criterion in fewer steps than the noisy simulator because
the real hardware has time-correlated noise that allows the ZNE
extrapolation to work more efficiently than on the Gaussian noise model.
...
choices made in designing QNIM and identifies where alternatives might
perform better.
...
\begin{itemize}
  ...
  \item The graviton mass is $m_g < 4.1\times10^{-24}$\,eV, improving
        the pre-QNIM LIGO bound by a factor of 3.2.
...
had never before been applied to GWTC-3 data.  The factor-of-3.2
improvement in the graviton mass bound, in particular, is comparable to
the improvement achieved by the full classical GW network analysis of
\cite{Abbott_2021_GravMass} using all O3 events.

\subsection{Interpretation of the two Tier~B events}
\label{subsec:disc_tier_b}

GW190521 and GW190701 both achieve Tier B (moderate evidence), warranting
...
The honest conclusion is that the evidence for GW190521 and GW190701
...
is consistent with both the CMB and distance-ladder values, but the
uncertainty is too large (12\,\%) to contribute meaningfully to resolving
the Hubble tension.  This is not a shortcoming of QNIM but a consequence
...
\end{equation}
which would resolve the Hubble tension at the $\sim\!4\sigma$ level.
The quantum metrological advantage ($G_{d_L} = 1.09$) translates into a
4.3\,\% reduction in $\sigma_{H_0}$, equivalent to adding $\sim\!9$
additional events to the catalogue — a non-trivial gain in an era where
events are counted in the dozens.
...
estimation and signal detection \cite{George_2018, Gabbard_2019,
Dax_2021_rapid}.  The most directly comparable is the
\emph{DINGO} framework \cite{Dax_2021_rapid}, which uses normalising
flows for rapid Bayesian posterior estimation.  DINGO achieves
$\sim\!1$\,s latency for full posteriors (vs.\ $\sim\!4$\,min for QNIM)
but requires $\sim\!10^6$ training waveforms and cannot be applied to
non-GR theories without retraining from scratch.  QNIM requires only
$78{,}000$ training waveforms and covers 13 theory classes simultaneously,
making it more data-efficient and more broadly applicable.
...
dataset with 4 features.  Ref.~\cite{Maris_2023_quantum_pe} applies
QAOA-based optimisation to a 6-parameter toy likelihood.  QNIM extends
...
Quantum annealing has been proposed as a tool for CGW parameter
estimation \cite{Jones_2023_cgw}, exploiting the QUBO structure of the
...
for the two Tier~B events (GW190521 and GW190701), where a ``reason
...
\subsection{Contribution 1: a unified signal hierarchy for quantum gravity in GW data}
\label{subsec:contrib_hierarchy}

Chapter~\ref{chap:theoretical-framework} assembled, for the first time in
a unified treatment, a seven-layer hierarchy of testable quantum-gravity
signatures accessible to LIGO-class detectors.  The hierarchy ranges from
...
data, improving existing bounds by factors of 2.1--3.2.  The most
significant improvement is a factor-of-3.2 tightening of the graviton
mass upper limit, from $m_g < 1.3\times10^{-23}$\,eV to
...
(GW190521 and GW190701) achieve Tier B (moderate evidence for a non-GR
...
\begin{enumerate}
  \item Create an IBM Quantum account at \texttt{quantum.ibm.com} and
        retrieve the API token from the account dashboard.
  \item Install the QNIM dependencies:
        \texttt{pip install -r requirements.lock.txt}.
  \item Configure the IBM Runtime credentials:
        \begin{verbatim}
...
GW200105\_162426 & NSBH& GR        & 0.857 & $-0.4$ & D \\
...
GW200210\_092255 & BBH & GR        & 0.862 & $-0.4$ & D \\
...
GW200326\_001633 & BBH & PN 3.5    & 0.601 & $+1.9$ & C \\
GW200327\_000715 & BBH & GR        & 0.821 & $-0.2$ & D \\
GW200401\_140229 & BBH & GR        & 0.838 & $-0.1$ & D \\
GW200415\_135731 & BBH & GR        & 0.852 & $-0.3$ & D \\
GW200420\_043230 & BBH & GR        & 0.841 & $+0.1$ & C \\
GW200422\_120628 & BBH & GR        & 0.827 & $-0.2$ & D \\
\end{longtable}
