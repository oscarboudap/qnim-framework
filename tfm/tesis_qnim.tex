...
(minimum $5.9\sigma$ across all five tests), establishing a
formal demonstration of quantum advantage in inference.
...
discussion in Appendix~\ref{app:feature-map-theory}.
...
the Imaginary Time Schrödinger Equation (ITSE) formalism:
\begin{equation}
  \label{eq:tunnelling}
...
(Table~\ref{tab:qmg_noisy}); contribution 5 (observational bounds)
...
Section~\ref{subsec:disc_model_selection}.
...
regions (high QFI, avoiding barren plateaus). In the Adam optimizer step
$\boldsymbol{\theta}_{t+1} = \boldsymbol{\theta}_t - a_t \hat{g}^\mathrm{total}_t$,
...
Objective~O2 (Section~\ref{sec:objectives}) uses Kullback–Leibler
...
The JSD values of Table~\ref{tab:injection_jsd} do not invalidate
...
Table~\ref{tab:param_est_summary} are derived from \emph{synthetic}
...
The H$_0$ measurement of Section~\ref{sec:cosmological_results}
...
spectral mass on directions that separate the twelve beyond-GR classes.
...
Table~\ref{tab:injection_jsd} applies to a different protocol
...
Although this thesis primarily focuses on GWTC-3 (O1–O3) events, the
...
(See Appendix~\ref{app:full_catalogue}, marked with $\dagger$ symbol,
...
improvement from the quantum advantage ($G_{d_L} = 1.09$, Table~\ref{tab:qmg_parameters})
...
\begin{itemize}
  ...
  \item \textbf{Theory scope.}  Dingo-T1 operates within GR: it
        ...
        theory-aware: each of the twelve beyond-GR classes contributes a
...
\begin{enumerate}[label=(\alph*),leftmargin=*]
  \item \textbf{Theory-agnostic ppE vs.\ 13 explicit theory classes.}
        ...
        hierarchy (Section~\ref{sec:qg_hierarchy}), including
...
\begin{itemize}
  ...
  \item The Berry--Keating--Connes conjecture~\cite{Berry_Keating_1999,Connes_1999}
        ...
        Appendix~\ref{app:bkc_speculative}; a falsifiable empirical test
...
defined in Section~\ref{sec:bayes_factor_formalism}, events with $\ln B \in [0,2]$
...
GW190521         & BBH$^\dagger\ddagger$ & Extra dim.     & \textbf{0.723} & $\mathbf{+3.8}$ & \textbf{B} \\
...
\begin{flushleft}
\footnotesize $^\dagger$~Events used in the QNIM Hubble constant measurement
(Section~\ref{sec:cosmological_results}); selected on the basis of QNIM
$d_L$ precision $<20\,\%$ and GLADE$+$ host galaxy candidate.
\end{flushleft}
\begin{flushleft}
\footnotesize $^\ddagger$~GW190521 is classified as BBH/IMBH; both components
lie in or above the pair-instability supernova mass gap \cite{Abbott_2020_GW190521}.
\end{flushleft}
...
Tier A: 0 events (0.0\,\%),
Tier B: 2 events (2.2\,\%),
Tier C: 22 events (24.4\,\%),
Tier D: 66 events (73.3\,\%).
...
% BIBLIOGRAPHY
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% CITATION NOTE: Dax_2021 refers to the NeurIPS 2021 conference
% paper (rapid PE); Dax_2021_rapid refers to the same work's
% arXiv preprint version. Throughout this document, Dax_2021_rapid
% is used uniformly to refer to this work.
