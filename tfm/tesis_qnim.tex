...
controlled injections spanning thirteen theory classes (General Relativity plus
twelve beyond-GR alternatives),
...
to full training convergence projects $91\,\%\;(\pm2\,\%)$ (estimated by
fitting a logarithmic learning curve $\mathrm{Acc}(t) = A - B \cdot e^{-t/\tau}$
to the 218-step validation accuracy trajectory, with bootstrap resampling
over 1000 resamplings of the last 50 steps; the $\pm 2\,\%$ interval is
the 95\,\% bootstrap confidence interval), but this figure was not
independently reproduced due to hardware budget constraints.  A formal
quantum advantage is demonstrated through a Quantum-to-Classical
Fisher Information ratio of $\mathcal{F}_Q/\mathcal{F}_C \in [1.75,\,2.23]$
for the critical beyond-GR parameters on the noise-model simulator.
However, on IBM Fez physical hardware after ZNE, only 2 of 5 critical
parameters retain this advantage ($G_k^{\mathrm{noisy}} > 1$); the others
suffer information loss due to current CNOT fidelities ($\epsilon_{2Q} \approx
3.8 \times 10^{-3}$). This hardware-level degradation is a feature of
present-day NISQ devices, not a fundamental limitation of the QNIM
methodology, and is extensively discussed in Section~\ref{subsec:disc-qfi-hardware}.
Re-analysis of GW150914 confirms full consistency with General Relativity
across thirteen competing models (as assessed by Bayes factor comparison;
see Section~\ref{subsec:gw150914} for parameter recovery details).
The statistical comparison of QNIM predictions against classical Wilcoxon
signed-rank tests for beyond-GR hypothesis discrimination
yields Bonferroni-corrected $p$-values in the range
$[2.1\times10^{-18},\,8.9\times10^{-9}]$, corresponding to
Gaussian-equivalent significances of $5.9\sigma$--$8.7\sigma$
(minimum $5.9\sigma$ across all five tests).
This represents a formal demonstration of quantum advantage in
inference, not a discovery claim of new physics. As a secondary contribution,
...
distanceinclination degeneracy through non-linear memory effects, reducing
...
Section~\ref{subsec:disc-model-selection}.
