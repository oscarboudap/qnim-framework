...
All six contributions are subject to the hardware limitations
discussed in Section~\ref{sec:disc_nisq}: contributions 1--4
...
as discussed in Section~\ref{sec:bayes_factor_formalism}.
...
where the noise model calibration to ibm_fez hardware introduces systematic
...
\begin{table}[htb]
  ...
  \caption{Marginal Jensen--Shannon divergence (nats) between QNIM Fisher
    ...
    The threshold from \citeA{Dax_2021_rapid} is $10^{-3}$\,nats.}
...
\noindent\multicolumn{6}{l}{\footnotesize $^\dagger$~Events used in the 
QNIM Hubble constant measurement (Section~\ref{sec:cosmological_results}); 
selected on the basis of QNIM $d_L$ precision $<20\,\%$ and 
GLADE$+$ host galaxy candidate.} \\[0.2em]
\noindent\multicolumn{6}{l}{\footnotesize $^\ddagger$~GW190521 is classified as BBH/IMBH; both components lie in or above the pair-instability supernova mass gap \cite{Abbott_2020_GW190521}.}
