...
GW150914 & BBH & 35.6 & 30.6 & 410  & 24.0 & High \\
...
metrological pyramid.  Chapter~\ref{chap:quantum-advantage} provides empirical confirmation
of this advantage on the noiseless simulator, with
$\mathcal{F}_Q/\mathcal{F}_C \in [1.75,\,2.23]$ for the five critical
beyond-GR parameters; under realistic IBM~Fez hardware conditions
only $\kappa_\mathrm{CS}$ and $R_c$ retain $G_k^\mathrm{noisy} > 1$
(Section~\ref{subsec:qfi-noisy}).
...
\begin{description}
  ...
  \item[Quantum metrological gains for beyond-GR parameters.]
        ...
        $G_k \in [1.75,\,2.23]$ for the five critical beyond-GR parameters,
        evaluated on the noiseless AerSimulator, yielding 25--33\,\% tighter
        estimation bounds than classical matched filtering at the same SNR\@.
        Under realistic IBM~Fez hardware conditions
        ($\mathcal{F}_\mathrm{circ} = 0.714$), this advantage is partially
        degraded: only $\kappa_\mathrm{CS}$ and $R_c$ retain
        $G_k^\mathrm{noisy} > 1$ at current fidelity; the remaining three
        parameters recover their advantage at
        $\mathcal{F}_\mathrm{circ} \geq 0.74$, projected for the next
        IBM~Heron generation (Table~\ref{tab:qmg-noisy}).
