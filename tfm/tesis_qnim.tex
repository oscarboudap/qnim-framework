...
is provided in Appendix~\ref{app:complexity-table}.

The end-to-end wall-clock latency of $\sim\!244$\,s per event on the hybrid 
IBM Fez + D-Wave A2 backend is dominated by VQC evaluation (Stage~4) and 
Bayes factor computation (Stage~6). The measured speedup relative to classical 
nested sampling ($\sim\!24$\,CPU-h per event) is $\sim\!15$--$350\times$ depending 
on available cores, while simultaneously delivering the metrological advantages 
of Chapter~\ref{chap:quantum-advantage}. The scalability roadmap—detailing 
...
\caption{Wilcoxon signed-rank test results for the five quantum
         ...
         in the text. These results quantify the advantage of QNIM
...
$W$-statistic & $p$-value (raw) & \textbf{Significant?} \\
\midrule
...
uniform random baseline.  \emph{Second}, the $21.4\times$ speedup in the
rapid-descent phase (25 simulator-mode steps, or 14 IBM-mode steps, to reach
loss $= 2.31$, versus $\sim\!300$ steps for standard SPSA to reach the same
threshold) is explained mathematically by the quantum natural gradient: where
...
\caption{QNIM 90\,\% upper limits on beyond-GR parameters from the
         combined analysis of 86 BBH events in GWTC-3.
         ...
         $\epsilon_k \in [\epsilon_k^\mathrm{min},\,
         \epsilon_k^\mathrm{max}]$. ``Prior bound'' is the
         pre-existing observational bound used to set
         $\epsilon_k^\mathrm{max}$ in the SSTG injection prior.
         Improvement factors compare the QNIM upper limit to the
         prior bound.}
...
These improvements have two independent sources. The first is
statistical: combining 86 events reduces the uncertainty by a factor
of approximately $\sqrt{N_\mathrm{events}/N_\mathrm{prev}}$ relative
to analyses based on fewer events. Most prior bounds used
1--10 events \cite{Abbott_2021_GWTC3_TGR}, giving an expected
statistical improvement of $\sqrt{86/10} \approx 2.9\times$.
...
% ------------------------------------------------------------------
\subsection{NISQ hardware constraints and their evolution}
\label{subsec:disc-nisq}
% ------------------------------------------------------------------
...
in scientific machine learning—not resting on asymptotic computational complexity 
arguments, but grounded in the physics of the problem itself. The quantum kernel 
genuinely resolves features (frequency-quadratic phase rotations, spectral 
broadening, discrete echo trains) that remain invisible to any classical kernel 
operating in the same 12-dimensional feature space. This resolves the central 
paradox that opened Chapter~\ref{chap:introduction}: how can quantum computing 
help if the classical computation is already optimal? The answer is that optimality 
in Euclidean space does not guarantee optimality in Hilbert space.

This thesis has presented the Quantum Native Inverse Models (QNIM)
methodology as a hybrid quantum-classical analytical framework for the
...
\subsection*{K.3 Analysis for the QNIM circuit}

For the QNIM configuration ($n = d = 12$, $r_\phi = 2$, cyclic nearest-neighbor
ZZ entanglement):

\paragraph{Condition: Linear (nearest-neighbor) ZZ entanglement, $r_\phi = 1$}
In this case, each ZZ gate crosses at most one bipartition boundary per cut,
bounding $S_L \leq 1$ bit across any cut. Thus $\chi_\mathrm{crit} = 2$,
placing the circuit firmly in the classically-simulable regime.

\paragraph{Condition: Multiple repetitions $r_\phi \geq 2$, cyclic topology}
After the first ZZ encoding layer, pairwise feature-angle correlations
$\phi_{\{i,j\}}(\mathbf{x}) = (\pi - x_i)(\pi - x_j)$ (Eq.~\ref{eq:phi_angles})
create two-body entanglement. After the Hadamard layer and second ZZ application,
composite terms emerge:
\begin{equation}
...
raising the effective Schmidt rank to:
\begin{equation}
  \label{eq:chi_crit_appendix}
  \chi_\mathrm{crit} = 2^{\lfloor n/2 \rfloor} = 2^{\lfloor 12/2 \rfloor} = 2^6 = 64.
\end{equation}

\subsection*{K.4 Scalability with qubit count}

As $n$ scales, the dequantisation threshold grows as $\chi_\mathrm{crit}(n) = 2^{\lfloor n/2 \rfloor}$,
growing exponentially. This means:
\begin{itemize}
  \item For $n = 12$: $\chi_\mathrm{crit} = 64$ (current QNIM).
  \item For $n = 14$: $\chi_\mathrm{crit} = 2^7 = 128$.
  \item For $n = 16$: $\chi_\mathrm{crit} = 2^8 = 256$.
  \item For $n = 18$: $\chi_\mathrm{crit} = 2^9 = 512$.
\end{itemize}

Classical MPS simulation with such bond dimensions becomes intractable (memory cost
$\propto n \chi^2 \sim 18 \times 512^2 \approx 4.7$ million real numbers), while
quantum simulation remains feasible on next-generation hardware.
...
\paragraph{MPS simulation at the threshold:}
An open-boundary MPS with $\chi = 64$ and $d_\mathrm{phys} = 1$ has $\sim 12 \times 64^2 \approx 3 \times 10^6$
real parameters. Contracting the kernel matrix element over $2^{12} = 4096$ basis states costs
$\sim 10^8$ FLOP per pair, or $\sim 10^{17}$ FLOP for the full training set  roughly
100$\times$ more than the quantum circuit approach.

\subsection*{K.6 Implications for quantum advantage}

The existence of $\chi_\mathrm{crit} = 64$ means that:
\begin{enumerate}
  \item Any classical MPS simulation with $\chi < 64$ is approximate, not exact.
  \item The QNIM quantum kernel cannot be efficiently emulated classically unless
        one is willing to expend exponentially large memory and time.
  \item As $n$ scales to 14--18 qubits (next-generation roadmap), the quantum
        advantage becomes asymptotically certifiable via MPS simulation impossibility.
\end{enumerate}

The proposed empirical follow-up (Section~\ref{subsec:dequantisation}) is to train
a classical MPS-64 classifier on the same data and compare its accuracy to QNIM.
If QNIM maintains an advantage, this constitutes proof of genuine quantum advantage
that survives the strongest known classical simulation strategy.
