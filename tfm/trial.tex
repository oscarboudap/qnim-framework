...
% ------------------------------------------------------------------
\section{QNIM: motivaci\'on y contribuciones}
% ------------------------------------------------------------------
\usepackage{amsmath,amssymb,amsthm}
% Layout and typography
The sections below retain only the compact argument needed in the
introduction.  The detailed discussion of the methodological claim,
the architectural justification, and the full scientific evaluation are
deferred to the later discussion chapters.
...
Classical models can approximate the data, but the beyond-GR signatures
of interest are encoded in higher-order correlations that become natural
only after lifting the problem into a quantum feature space.  In that
space, the relevant phase structure is evaluated simultaneously rather
than reconstructed monomial by monomial, so the representational burden
does not scale like a classical feature expansion.  The metrological
argument is independent: the Quantum Cram\'er--Rao Bound still places a
tighter limit on achievable precision whenever the quantum Fisher
information exceeds the classical one.  Quantum computing is therefore
not a performance add-on but the mechanism that makes the intended
inference strategy physically accessible.

% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% CHAPTER 1 — INTRODUCTION
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
The choice of 12 qubits is not arbitrary: it is the point where five
restrictions act simultaneously, and the full derivation is given in
Section~8.2.1.  In brief, 12 qubits is the smallest configuration that
remains expressive enough for the data, separable enough for the class
structure, stable enough for trainability, faithful enough for current
hardware, and hard enough to justify the quantum approach.
...
\end{equation}
\begin{enumerate}
  \item Unified signal hierarchy for quantum-gravity signatures in gravitational-wave data.
  \item QNSPSA-EML-Feynman training algorithm with improved trainability.
  \item Quantum metrological gains for beyond-GR parameter estimation.
  \item SSTG synthetic data generator for labeled multimodal training sets.
  \item New observational bounds from the GWTC-3 catalogue.
  \item Production-grade quantum software architecture for reproducible NISQ workflows.
\end{enumerate}
